...
from komicove_backend.accounts.email_delivery import send_account_email
from komicove_backend.accounts.models import AccountActionToken, User, _now
...
def send_verification(db: Session, user: User) -> bool:
    ...
    return send_account_email(user.email, "Komicove - confirme seu e-mail / verify your email",
        "PT: Cole este código no Komicove para confirmar seu e-mail:\n\n"
        f"{token}\n\nEN: Paste this code into Komicove to verify your email.\n"
...
def send_password_reset(db: Session, user: User) -> bool:
    ...
    return send_account_email(user.email, "Komicove - recuperação de senha / password recovery",
        "PT: Cole este código no Komicove para criar uma nova senha:\n\n"
        f"{token}\n\nEN: Paste this code into Komicove to create a new password.\n"
