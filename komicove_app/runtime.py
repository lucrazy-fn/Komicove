...
def set_app_icon(window):
    try:
        if sys.platform == "win32":
            window.iconbitmap(resource_path("Komicove.ico"))
        else:
            icon = tk.PhotoImage(file=resource_path("komicovelogo.png")).subsample(24)
            window.iconphoto(True, icon)
            window._komicove_icon = icon
    except Exception as exc:
        logging.getLogger("komicove").debug("Could not set window icon: %s", exc)
...
def grab_when_visible(window):
    if sys.platform == "win32":
        window.grab_set()
        return
    def acquire(event=None):
        if event is not None and event.widget is not window:
            return
        if window.winfo_exists() and window.winfo_viewable():
            window.grab_set()
            if binding[0]:
                window.unbind("<Visibility>", binding[0])
                binding[0] = None
    binding = [window.bind("<Visibility>", acquire, add="+")]
    window.after_idle(acquire)
...
from komicove_app.account_views import render_notifications, render_profile
from komicove_app.sync import build_sync_payload, content_id
from komicove_app.reader import (load_state as load_reader_state, save_state as save_reader_state,
    load_manual_panels, save_manual_panels)
from komicove_app.downloads import manager as download_manager, render_downloads
from komicove_app.community import load_catalog, save_catalog
from komicove_app.moderation import can_moderate
from komicove_app.auth import role_label

logging.basicConfig(level=logging.WARNING, format="%(levelname)s %(name)s: %(message)s")
log = logging.getLogger("komicove")
...
def resource_path(relative_path):
    try:
        ...
        base = "/app" if Path("/.flatpak-info").exists() else str(Path(__file__).resolve().parent.parent)
...
from komicove_app.themes import DARK as DARK, LIGHT as LIGHT, TEXTS as TEXTS
...
from komicove_app.archive import (
...
try:
    from komicove_client import api_client, session_store
