...
final class ReaderPreferences {
    static void show(Activity activity, Runnable changed) {
        ...
        for(int i=0;i<keys.length;i++) {
            checks[i]=new CheckBox(activity); checks[i].setText(I18n.t(activity,labels[i]));
        ...
        new PanelDialog(activity).setTitle("Preferências do leitor").setView(box)
            .setPositiveButton(I18n.t(activity,"Salvar"),(d,w)->{
                SharedPreferences.Editor edit=prefs.edit();
                for(int i=0;i<keys.length;i++)edit.putBoolean(keys[i],checks[i].isChecked());
                edit.apply();if(changed!=null)changed.run();
            }).setNegativeButton(I18n.t(activity,"Cancelar"),null).show();
    }
}
