...
final class Ui {
    ...
    static TextView text(Context c,String s,int size,int color) {
        TextView t=new TextView(c); t.setText(s); t.setTextSize(size); t.setTextColor(color); t.setPadding(0,dp(c,5),0,dp(c,5)); return t;
    }
    ...
    static Button button(Context c,String s,Runnable action) {
        Button b=new Button(c); b.setText(s); b.setAllCaps(false); b.setTextColor(TEXT); b.setTextSize(14);
    ...
    static EditText input(Context c,String hint,boolean password) {
        EditText e=new EditText(c); e.setHint(hint); e.setTextColor(TEXT); e.setHintTextColor(MUTED); e.setSingleLine(true);
