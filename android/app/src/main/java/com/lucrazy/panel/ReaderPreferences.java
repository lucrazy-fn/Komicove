...
final class ReaderPreferences {
    static void show(Activity activity, Runnable changed) {
        ...
        String[] keys={"persist_zoom","auto_fit","guided"};
        String[] labels={"Manter zoom e posição entre páginas","Encaixar a página ao abrir","Leitura guiada — experimental"};
        ...
        new AlertDialog.Builder(activity).setTitle("Preferências do leitor").setView(box)
            .setPositiveButton("Salvar",(d,w)->{
                ...
                for(int i=0;i<3;i++)edit.putBoolean(keys[i],checks[i].isChecked());
