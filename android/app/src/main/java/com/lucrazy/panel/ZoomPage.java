...
final class ZoomPage extends View {
    ...
    Bitmap image;float zoom=1,panX,panY;private float downX,downY,lastX,lastY;private boolean pinching,moved;
    ...
    ZoomPage(Context c,Actions a){super(c);actions=a;setContentDescription("Página do quadrinho. Use dois dedos para ampliar; deslize para trocar de página.");setFocusable(true);
        ...
        gestures=new GestureDetector(c,new GestureDetector.SimpleOnGestureListener(){public boolean onDown(android.view.MotionEvent e){return true;}
            ...
            public boolean onSingleTapConfirmed(MotionEvent e){if(e.getX()<getWidth()*.22)actions.previous();else if(e.getX()>getWidth()*.78)actions.next();else actions.toggle();performClick();return true;}});
    }
    ...
    @Override public boolean onTouchEvent(MotionEvent e){scale.onTouchEvent(e);gestures.onTouchEvent(e);
        ...
        if(e.getActionMasked()==MotionEvent.ACTION_MOVE&&e.getPointerCount()==1&&!scale.isInProgress()){if(zoom>1){panX+=e.getX()-lastX;panY+=e.getY()-lastY;clamp();invalidate();}lastX=e.getX();lastY=e.getY();moved=true;}
