...
final class LibraryStore {
    static class Book {
        String id,title,file,collection="";int page,count;boolean favorite;double updated;float zoom=1,offsetX=0,offsetY=0;String mode="normal";JSONArray marks=new JSONArray();
        JSONObject json()throws JSONException{return new JSONObject().put("id",id).put("title",title).put("file",file).put("collection",collection).put("page",page).put("count",count).put("favorite",favorite).put("updated",updated).put("zoom",zoom).put("offsetX",offsetX).put("offsetY",offsetY).put("mode",mode).put("marks",marks);}
        static Book parse(JSONObject j){Book b=new Book();b.id=j.optString("id");b.title=j.optString("title");b.file=j.optString("file");b.collection=j.optString("collection");b.page=j.optInt("page");b.count=j.optInt("count");b.favorite=j.optBoolean("favorite");b.updated=j.optDouble("updated",0);b.zoom=(float)j.optDouble("zoom",1);b.offsetX=(float)j.optDouble("offsetX",0);b.offsetY=(float)j.optDouble("offsetY",0);b.mode=j.optString("mode","normal");b.marks=j.optJSONArray("marks");if(b.marks==null)b.marks=new JSONArray();return b;}
    }
